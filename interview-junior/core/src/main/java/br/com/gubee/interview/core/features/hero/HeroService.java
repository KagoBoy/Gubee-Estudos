...
public class HeroService {
    ...
    public Hero updateById(CreateHeroRequest createHeroRequest, UUID id) {
        ...
        return heroRepository.findById(id)
                .orElseThrow(() -> new HeroNotFoundException("Hero not found after update with id: " + id));
    }
    ...
    public Hero updateByName(CreateHeroRequest createHeroRequest, String name) {
        ...
        return heroRepository.findByName(name)
                .orElseThrow(() -> new HeroNotFoundException("Hero not found after update with name: " + name));
    }
    ...
    public Hero findByName(String name){
        return heroRepository.findByName(name).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Hero not found"));
    }
    ...
    public ComparisonResponse compareHeroes (UUID hero1Id, UUID hero2Id){
        Hero hero1 = heroRepository.findById(hero1Id).orElseThrow(() -> new HeroNotFoundException("Hero not found with id: " + hero1Id));
        Hero hero2 = heroRepository.findById(hero1Id).orElseThrow(() -> new HeroNotFoundException("Hero not found with id: " + hero2Id));

        PowerStats stats1 = powerStatsRepository.findById(hero1.getPowerStatsId()).orElseThrow(() -> new PowerStatsNotFoundException("Power Stats from " + hero1Id + " not found"));
        PowerStats stats2 = powerStatsRepository.findById(hero2.getPowerStatsId()).orElseThrow(() -> new PowerStatsNotFoundException("Power Stats from " + hero2Id + " not found"));
